...
let timeout;
hero.addEventListener('mousemove', ({x, y}) => {
  if (timeout) {
...
// Hero Background
const colors = ["#3CC157", "#2AA7FF", "#1B1B1B", "#FCBC0F", "#4b81d7", "#F85F36"];
// const colors = ["#3CC157", "#2AA7FF", "#1B1B1B", "#FCBC0F", "#4b81d7"];
...
for (let i = 0; i < numBalls; i++) {
  ...
  // ball.style.left = `${Math.floor(Math.random() * 10)}vw`;
  // ball.style.top = `${Math.floor(Math.random() * 10)}vh`;
  ball.style.left = `${Math.floor(Math.random() * (75 - 25) + 25)}%`;
  ball.style.top = `${Math.floor(Math.random() * (60 - 20) + 20)}%`;
  ball.style.transform = `scale(${Math.random()})`;
  ball.style.width = `${Math.random() * 3}em`;
  ball.style.height = ball.style.width;

  balls.push(ball);
  document.body.querySelector('.heroBg').append(ball);
}
